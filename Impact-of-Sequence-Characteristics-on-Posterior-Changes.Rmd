...
synth.cat.df %>%
  ...
  facet_wrap(~idx, scales = "fixed", ncol = 2, labeller = as_labeller(function(x) {
    paste0(
      ...
      ", N.bkg = ", unique(synth.cat.df$N.bkg[synth.cat.df$idx == x]), ", N.after = ",
...
plot.mp <- ggplot(bind.post.df, aes(x = x, y = y, color = cat.used, linetype = cat.used)) +
  geom_line(size = 1) +
  ...
  labs(
    color = "Category Used",
    linetype = "Category Used"
  )

plot.mp
ggsave("marginal_posterior_distributions.pdf", plot.mp, width = 15, height = 10, dpi = 300)
```
